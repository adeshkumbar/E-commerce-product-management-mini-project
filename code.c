...
int main()
{
    ...
    while (1)
    {
        ...
        {
            int id;
            printf("Enter product ID to add to order queue: ");
            scanf("%d", &id);
            Product *p = findProductById(productList, id);
            if (p)
            {
                enqueueOrder(&orderQueueFront, &orderQueueRear, p);
        ...
        case 6:
            dequeueOrder(&orderQueueFront, &orderQueueRear);
            break;
        case 7:
            printQueue(orderQueueFront);
            break;
        case 8:
...
void enqueueOrder(Queue **front, Queue **rear, Product *product)
{
    ...
    if (!newOrder) return;
    newOrder->product = copyProduct(product);
    newOrder->next = NULL;
    if (*rear == NULL)
    {
        *front = *rear = newOrder;
